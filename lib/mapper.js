...
exports.group = function(group){
  ...
  if (traits && traits.groupType) {
    var payload = {
      endpoint: '/groupidentify',
      data: {
        group_type: traits.groupType,
        group_value: groupId,
        group_properties: pickBy(traits, function(value, key){
          return key !== 'id' && key !== 'groupType';
        })
      }
    };
    return payload;
  } else {
    // legacy behavior with no group type
    var payload = {
      endpoint: '/identify',
      data: {
